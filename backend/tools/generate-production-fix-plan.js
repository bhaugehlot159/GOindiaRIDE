/* eslint-disable no-console */
const fs = require('fs');
const path = require('path');

function parseArgs(argv) {
  const args = {};
  for (let i = 0; i < argv.length; i += 1) {
    const token = String(argv[i] || '');
    if (!token.startsWith('--')) continue;
    const key = token.slice(2);
    const next = argv[i + 1];
    if (!next || String(next).startsWith('--')) {
      args[key] = true;
      continue;
    }
    args[key] = next;
    i += 1;
  }
  return args;
}

function normalizeBase(value, fallback = '') {
  const raw = String(value || fallback || '').trim();
  if (!raw) return '';
  return raw.replace(/\/$/, '');
}

function toHost(value, fallback = '') {
  try {
    return new URL(String(value || fallback || '')).hostname;
  } catch (_error) {
    return '';
  }
}

function buildReport({ siteBase, apiBase, adminEmail }) {
  const siteHost = toHost(siteBase, 'https://goindiaride.in');
  const apiHost = toHost(apiBase, 'https://api.goindiaride.in');

  return `# GOIndiaRIDE Production Fix Plan (Auto-Generated)

Generated At: ${new Date().toISOString()}
Site Host: ${siteHost}
API Host: ${apiHost}
Admin Email: ${adminEmail}

## 1) Backend env update
\`\`\`bash
cd /var/www/GOindiaRIDE/backend
cp .env .env.backup.$(date +%Y%m%d-%H%M%S)
sed -i 's|^CORS_ORIGIN=.*|CORS_ORIGIN=https://${siteHost}|' .env
sed -i 's|^SECURITY_ALLOWED_ORIGINS=.*|SECURITY_ALLOWED_ORIGINS=https://${siteHost},https://www.${siteHost.replace(/^www\./, '')}|' .env
grep -q '^BOOKING_ADMIN_ALERT_EMAILS=' .env && sed -i 's|^BOOKING_ADMIN_ALERT_EMAILS=.*|BOOKING_ADMIN_ALERT_EMAILS=${adminEmail}|' .env || echo 'BOOKING_ADMIN_ALERT_EMAILS=${adminEmail}' >> .env
grep -q '^DEFAULT_ADMIN_ALERT_EMAIL=' .env && sed -i 's|^DEFAULT_ADMIN_ALERT_EMAIL=.*|DEFAULT_ADMIN_ALERT_EMAIL=${adminEmail}|' .env || echo 'DEFAULT_ADMIN_ALERT_EMAIL=${adminEmail}' >> .env
\`\`\`

## 2) Nginx proxy fix for website /api (removes HTTP 405)
\`\`\`bash
sudo tee /etc/nginx/sites-available/goindiaride-site >/dev/null <<'NGINX'
server {
    listen 80;
    server_name ${siteHost} www.${siteHost.replace(/^www\./, '')};

    root /var/www/GOindiaRIDE;
    index index.html;

    location / {
        try_files $uri $uri/ /index.html;
    }

    location /api/ {
        proxy_pass http://127.0.0.1:5000/api/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }
}
NGINX

sudo ln -sf /etc/nginx/sites-available/goindiaride-site /etc/nginx/sites-enabled/goindiaride-site
sudo nginx -t
sudo systemctl reload nginx
\`\`\`

## 3) Optional api subdomain proxy (recommended)
\`\`\`bash
sudo tee /etc/nginx/sites-available/goindiaride-api >/dev/null <<'NGINX'
server {
    listen 80;
    server_name ${apiHost};

    location / {
        proxy_pass http://127.0.0.1:5000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }
}
NGINX

sudo ln -sf /etc/nginx/sites-available/goindiaride-api /etc/nginx/sites-enabled/goindiaride-api
sudo nginx -t
sudo systemctl reload nginx
\`\`\`

## 4) DNS checks (Cloudflare/Registrar)
\`\`\`bash
nslookup ${apiHost}
nslookup ${siteHost}
\`\`\`

## 5) Restart backend and verify
\`\`\`bash
cd /var/www/GOindiaRIDE/backend
npm install
npm run pm2:restart
pm2 status
pm2 logs goindiaride-backend --lines 50
\`\`\`

## 6) End-to-end tests
\`\`\`bash
cd /var/www/GOindiaRIDE/backend
npm run smtp:test-admin -- --to ${adminEmail}
npm run diagnose:live -- --site https://${siteHost} --api https://${apiHost} --origin https://${siteHost}
cat /var/www/GOindiaRIDE/backend/reports/production-live-diagnose-latest.json
\`\`\`

## Success criteria
1. \`smtp:test-admin\` successful with messageId.
2. \`diagnose:live\` route checks no longer show \`405\` on \`/api/bookings/fallback/admin-alert-email\`.
3. Booking submit from website shows no \`Admin Email Pending\` toast.
`;
}

function run() {
  const args = parseArgs(process.argv.slice(2));
  const siteBase = normalizeBase(args.site, 'https://goindiaride.in');
  const apiBase = normalizeBase(args.api, 'https://api.goindiaride.in');
  const adminEmail = String(args.email || 'bhaugehlot159@gmail.com').trim().toLowerCase();

  const report = buildReport({
    siteBase,
    apiBase,
    adminEmail
  });

  const outDir = path.join(__dirname, '..', 'reports');
  const outPath = path.join(outDir, 'production-fix-plan-latest.md');
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(outPath, report, 'utf8');

  console.log('Production fix plan generated:', outPath);
}

run();

