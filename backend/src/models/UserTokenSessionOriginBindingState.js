...
const userTokenSessionOriginBindingStateSchema = new mongoose.Schema({
  sidHash: { type: String, required: true, unique: true, index: true, trim: true },
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  boundOriginHostHash: { type: String, default: '', trim: true },
  boundRefererHostHash: { type: String, default: '', trim: true },
  ...
  lastOriginHostHash: { type: String, default: '', trim: true },
  lastRefererHostHash: { type: String, default: '', trim: true },
...
userTokenSessionOriginBindingStateSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });
userTokenSessionOriginBindingStateSchema.index({ status: 1, updatedAt: -1 });
userTokenSessionOriginBindingStateSchema.index({ userId: 1, status: 1, updatedAt: -1 });

module.exports = mongoose.model('UserTokenSessionOriginBindingState', userTokenSessionOriginBindingStateSchema);