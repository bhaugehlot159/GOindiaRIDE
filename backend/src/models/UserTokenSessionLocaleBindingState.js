...
const userTokenSessionLocaleBindingStateSchema = new mongoose.Schema({
  sidHash: { type: String, required: true, unique: true, index: true, trim: true },
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  boundLocaleHash: { type: String, default: '', trim: true },
  boundLanguageTag: { type: String, default: '', trim: true },
  status: {
    type: String,
  ...
  lastLocaleHash: { type: String, default: '', trim: true },
  lastLanguageTag: { type: String, default: '', trim: true },
...
userTokenSessionLocaleBindingStateSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });
userTokenSessionLocaleBindingStateSchema.index({ status: 1, updatedAt: -1 });
userTokenSessionLocaleBindingStateSchema.index({ userId: 1, status: 1, updatedAt: -1 });

module.exports = mongoose.model('UserTokenSessionLocaleBindingState', userTokenSessionLocaleBindingStateSchema);