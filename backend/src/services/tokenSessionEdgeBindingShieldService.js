...
const UserTokenSessionEdgeBindingState = require('../models/UserTokenSessionEdgeBindingState');
...
function resolveOptions() {
  const methods = splitCsv(env.tokenSessionEdgeBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionEdgeBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionEdgeBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionEdgeBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionEdgeBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionEdgeBindingRequireSidClaim),
    requireHostSignal: Boolean(env.tokenSessionEdgeBindingRequireHostSignal),
    requireProtoSignal: Boolean(env.tokenSessionEdgeBindingRequireProtoSignal),
    requireAsnSignal: Boolean(env.tokenSessionEdgeBindingRequireAsnSignal),
    requireNetworkOrgSignal: Boolean(env.tokenSessionEdgeBindingRequireNetworkOrgSignal),
    compareAsn: Boolean(env.tokenSessionEdgeBindingCompareAsn),
    compareNetworkOrg: Boolean(env.tokenSessionEdgeBindingCompareNetworkOrg),
    compareHost: Boolean(env.tokenSessionEdgeBindingCompareHost),
    compareProto: Boolean(env.tokenSessionEdgeBindingCompareProto),
    compareIpPrefix: Boolean(env.tokenSessionEdgeBindingCompareIpPrefix),
    ipPrefixOctets: normalizeNumber(env.tokenSessionEdgeBindingIpPrefixOctets, 3, 1, 8),
    windowMs: normalizeNumber(env.tokenSessionEdgeBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionEdgeBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionEdgeBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionEdgeBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionEdgeBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionEdgeBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionEdgeBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionEdgeBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionEdgeBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionEdgeBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function evaluateEdgeBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
...
function pickPrimaryReason(reasons = []) {
  ...
  return 'session_edge_binding_verified';
}

async function recordSessionEdgeBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_edge_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_edge_binding_shield_triggered',
          note: payload.note || 'Token session edge binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionEdgeBinding({ user = null, payload = null, req = null } = {}) {
  ...
  if (options.requireSidClaim && !context.sid) {
    await recordSessionEdgeBindingIncident(req, {
      eventType: 'session_edge_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session edge binding rejected token without sid',
  ...
  const existing = await UserTokenSessionEdgeBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluateEdgeBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionEdgeBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          ...
          lastReason: evaluation.detected ? primaryReason : 'session_edge_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_edge_binding_verified' };
    }

    await recordSessionEdgeBindingIncident(req, {
      eventType: 'session_edge_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session edge binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionEdgeBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastIpPrefixHash: context.ipPrefixHash,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_edge_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_edge_binding_quarantined',
    req,
    metadata: {
      sidHash,
      reasons: evaluation.reasons
  ...
  await recordSessionEdgeBindingIncident(req, {
    eventType: 'session_edge_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated session edge binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionEdgeBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionEdgeBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionEdgeBindingState.find(query)
...
async function releaseTokenSessionEdgeBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionEdgeBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionEdgeBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_edge_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionEdgeBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionEdgeBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_edge_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionEdgeBinding,
  isTokenSessionEdgeBindingSha256,
  getTokenSessionEdgeBindingShieldSnapshot,
  releaseTokenSessionEdgeBindingShieldStates,
