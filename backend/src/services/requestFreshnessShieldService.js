...
function autoGeneratedRequestId(userId, path, method) {
  const seed = `${String(userId || '')}:${String(path || '')}:${String(method || '')}:${Date.now()}:${Math.random().toString(36).slice(2, 14)}`;
  return `rf_${hashValue(seed).slice(0, 48)}_${Date.now().toString(36)}`.slice(0, 120);
}

function canInjectFreshnessHeaders(req, userId) {
  if (!userId) return false;
  const csrfHeader = String(req?.headers?.['x-csrf-token'] || req?.headers?.['x-xsrf-token'] || '').trim();
  const authHeader = String(req?.headers?.authorization || '').trim();
  return Boolean(csrfHeader) && /^bearer\s+/i.test(authHeader);
}

function parseFreshnessHeaders(req, context = {}) {
  ...
  const path = normalizePath(context.path || req?.originalUrl || req?.path || '/');
  const method = String(context.method || req?.method || 'GET').trim().toUpperCase();

  let resolvedRequestId = requestIdRaw;
  let resolvedTimestamp = Number.isFinite(timestampValue) ? Math.round(timestampValue) : null;
  let injected = false;

  if (canInjectFreshnessHeaders(req, userId)) {
    if (!resolvedRequestId) {
      resolvedRequestId = autoGeneratedRequestId(userId, path, method);
      injected = true;
    }
  ...
  return {
    ...
    requestIdHash: resolvedRequestId ? hashValue(resolvedRequestId) : '',
    timestampMs: Number.isFinite(resolvedTimestamp) ? Math.round(resolvedTimestamp) : null,
    injected
  };
}

function isHeaderOnlyViolation(existing) {
  const reasons = Array.isArray(existing?.metadata?.reasons) ? existing.metadata.reasons : [];
  ...
  const allowed = new Set([
    ...
    'request_timestamp_skew_exceeded'
  ]);

  return reasons.every((reason) => allowed.has(String(reason || '').trim().toLowerCase()));
}

function canAutoRecoverQuarantine({ existing, headers, options, now }) {
  if (!existing || existing.status !== 'quarantined') return false;
  if (!isHeaderOnlyViolation(existing)) return false;
  if (!headers.injected) return false;
  if (!headers.requestId) return false;

  const idLen = String(headers.requestId).length;
  if (idLen < options.requestIdMinLength || idLen > options.requestIdMaxLength) return false;
  if (!Number.isFinite(Number(headers.timestampMs))) return false;

  return Math.abs(now - Number(headers.timestampMs)) <= options.maxSkewMs;
}
...
async function inspectUserRequestFreshness({ user = null, req = null } = {}) {
  ...
  const headers = parseFreshnessHeaders(req, { userId, path, method });
  ...
  if (existing) {
    ...
    if (existing.status === 'quarantined' && quarantineUntilMs > now) {
      if (canAutoRecoverQuarantine({ existing, headers, options, now })) {
        await UserRequestFreshnessState.updateOne(
          { userId },
          {
            $set: {
              ...
              lastReason: 'request_freshness_auto_recovered',
              metadata: sanitizeMetadata({
                reasons: ['auto_recovered_with_server_injected_headers'],
                reasonMeta: { injected: true },
                rememberedRequestIds: {},
                violationCount: 0,
