...
const UserTokenSessionLocaleBindingState = require('../models/UserTokenSessionLocaleBindingState');
...
function parseAcceptLanguage(value) {
  ...
  return normalizeLocaleTag(noQuality);
}

function resolveOptions() {
  const methods = splitCsv(env.tokenSessionLocaleBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionLocaleBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionLocaleBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionLocaleBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionLocaleBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionLocaleBindingRequireSidClaim),
    requireLocaleSignal: Boolean(env.tokenSessionLocaleBindingRequireLocaleSignal),
    requireLanguageSignal: Boolean(env.tokenSessionLocaleBindingRequireLanguageSignal),
    compareLocale: Boolean(env.tokenSessionLocaleBindingCompareLocale),
    compareLanguage: Boolean(env.tokenSessionLocaleBindingCompareLanguage),
    allowLanguageRegionFallback: Boolean(env.tokenSessionLocaleBindingAllowLanguageRegionFallback),
    windowMs: normalizeNumber(env.tokenSessionLocaleBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionLocaleBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionLocaleBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionLocaleBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionLocaleBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionLocaleBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionLocaleBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionLocaleBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionLocaleBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionLocaleBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function parseLocaleContext({ user = null, payload = {}, req = null }) {
  ...
  const headerLocale = normalizeLocaleTag(
    firstHeaderValue(req, ['x-client-locale', 'x-locale', 'locale'])
  );
  const acceptLanguageLocale = parseAcceptLanguage(firstHeaderValue(req, ['accept-language']));
  const payloadLocale = normalizeLocaleTag(payload?.locale || payload?.language || payload?.lang || '');

  const localeTag = headerLocale || payloadLocale || acceptLanguageLocale;
  const localeHash = localeTag ? hashValue(localeTag) : '';
  const languageTag = normalizeLanguageTag(localeTag || payload?.lang || payload?.language || '');
...
function evaluateLanguageMatch({ existingLanguage = '', currentLanguage = '', options }) {
  if (!options.compareLanguage) return true;
  if (!existingLanguage || !currentLanguage) return true;
  if (existingLanguage === currentLanguage) return true;

  if (options.allowLanguageRegionFallback) {
    const a = normalizeLanguageTag(existingLanguage);
    const b = normalizeLanguageTag(currentLanguage);
...
function evaluateLocaleBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
  ...
  if (options.requireLocaleSignal && !context.localeTag) {
    reasons.push('missing_locale_signal');
    missingClaimCount += 1;
  }
  if (options.requireLanguageSignal && !context.languageTag) {
    reasons.push('missing_language_signal');
    missingClaimCount += 1;
  }
  ...
  if (existing) {
    ...
    if (
      options.compareLocale
      && context.localeHash
      && existing.boundLocaleHash
      && String(existing.boundLocaleHash) !== context.localeHash
    ) {
      reasons.push('sid_locale_binding_drift');
    }

    const languageMatch = evaluateLanguageMatch({
      existingLanguage: normalizeLower(existing.boundLanguageTag || '', 40),
      currentLanguage: normalizeLower(context.languageTag || '', 40),
      options
    });
    if (!languageMatch) {
      reasons.push('sid_language_binding_drift');
    }
  }
  ...
  metadata.localeHash = context.localeHash;
  metadata.languageTag = context.languageTag;
...
function pickPrimaryReason(reasons = []) {
  ...
  if (reasons.includes('sid_locale_binding_drift')) return 'sid_locale_binding_drift';
  if (reasons.includes('sid_language_binding_drift')) return 'sid_language_binding_drift';
  if (reasons.includes('missing_sid_claim')) return 'missing_sid_claim';
  if (reasons.includes('missing_locale_signal')) return 'missing_locale_signal';
  if (reasons.includes('missing_language_signal')) return 'missing_language_signal';
  if (reasons.length > 0) return reasons[0];
  return 'session_locale_binding_verified';
}

async function recordSessionLocaleBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_locale_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_locale_binding_shield_triggered',
          note: payload.note || 'Token session locale binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionLocaleBinding({ user = null, payload = null, req = null } = {}) {
  ...
  const context = parseLocaleContext({ user, payload: payload || {}, req });
  if (!context.userId) {
    return { ok: true, reason: 'missing_user' };
  }

  if (options.requireSidClaim && !context.sid) {
    await recordSessionLocaleBindingIncident(req, {
      eventType: 'session_locale_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session locale binding rejected token without sid',
  ...
  const existing = await UserTokenSessionLocaleBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluateLocaleBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionLocaleBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          boundLocaleHash: existing?.boundLocaleHash || context.localeHash,
          boundLanguageTag: existing?.boundLanguageTag || context.languageTag,
          ...
          lastLocaleHash: context.localeHash,
          lastLanguageTag: context.languageTag,
          lastJtiHash: context.jtiHash,
          lastReason: evaluation.detected ? primaryReason : 'session_locale_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_locale_binding_verified' };
    }

    await recordSessionLocaleBindingIncident(req, {
      eventType: 'session_locale_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session locale binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionLocaleBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastLanguageTag: context.languageTag,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_locale_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_locale_binding_quarantined',
    req,
    metadata: {
      sidHash,
      path,
  ...
  await recordSessionLocaleBindingIncident(req, {
    eventType: 'session_locale_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated session locale binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionLocaleBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionLocaleBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionLocaleBindingState.find(query)
  ...
  return {
    ...
    items: rows.map((item) => ({
      ...
      boundLocaleHash: String(item.boundLocaleHash || ''),
      boundLanguageTag: String(item.boundLanguageTag || ''),
...
async function releaseTokenSessionLocaleBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionLocaleBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionLocaleBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_locale_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionLocaleBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionLocaleBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_locale_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionLocaleBinding,
  isTokenSessionLocaleBindingSha256,
  getTokenSessionLocaleBindingShieldSnapshot,
  releaseTokenSessionLocaleBindingShieldStates,
  quarantineTokenSessionLocaleBinding
};