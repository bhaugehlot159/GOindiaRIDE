...
const UserTokenSessionClientBindingState = require('../models/UserTokenSessionClientBindingState');
...
function resolveOptions() {
  const methods = splitCsv(env.tokenSessionClientBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionClientBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionClientBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionClientBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionClientBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionClientBindingRequireSidClaim),
    requireHostSignal: Boolean(env.tokenSessionClientBindingRequireHostSignal),
    requireProtoSignal: Boolean(env.tokenSessionClientBindingRequireProtoSignal),
    requireAsnSignal: Boolean(env.tokenSessionClientBindingRequireAsnSignal),
    requireNetworkOrgSignal: Boolean(env.tokenSessionClientBindingRequireNetworkOrgSignal),
    compareAsn: Boolean(env.tokenSessionClientBindingCompareAsn),
    compareNetworkOrg: Boolean(env.tokenSessionClientBindingCompareNetworkOrg),
    compareHost: Boolean(env.tokenSessionClientBindingCompareHost),
    compareProto: Boolean(env.tokenSessionClientBindingCompareProto),
    compareIpPrefix: Boolean(env.tokenSessionClientBindingCompareIpPrefix),
    ipPrefixOctets: normalizeNumber(env.tokenSessionClientBindingIpPrefixOctets, 3, 1, 8),
    windowMs: normalizeNumber(env.tokenSessionClientBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionClientBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionClientBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionClientBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionClientBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionClientBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionClientBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionClientBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionClientBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionClientBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function evaluateClientBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
...
function pickPrimaryReason(reasons = []) {
  ...
  return 'session_client_binding_verified';
}

async function recordSessionClientBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_client_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_client_binding_shield_triggered',
          note: payload.note || 'Token session client binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionClientBinding({ user = null, payload = null, req = null } = {}) {
  ...
  if (options.requireSidClaim && !context.sid) {
    await recordSessionClientBindingIncident(req, {
      eventType: 'session_client_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session client binding rejected token without sid',
  ...
  const existing = await UserTokenSessionClientBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluateClientBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionClientBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          ...
          lastReason: evaluation.detected ? primaryReason : 'session_client_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_client_binding_verified' };
    }

    await recordSessionClientBindingIncident(req, {
      eventType: 'session_client_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session client binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionClientBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastIpPrefixHash: context.ipPrefixHash,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_client_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_client_binding_quarantined',
    req,
    metadata: {
      sidHash,
      reasons: evaluation.reasons
  ...
  await recordSessionClientBindingIncident(req, {
    eventType: 'session_client_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated session client binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionClientBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionClientBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionClientBindingState.find(query)
...
async function releaseTokenSessionClientBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionClientBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionClientBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_client_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionClientBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionClientBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_client_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionClientBinding,
  isTokenSessionClientBindingSha256,
  getTokenSessionClientBindingShieldSnapshot,
  releaseTokenSessionClientBindingShieldStates,
  quarantineTokenSessionClientBinding
};
