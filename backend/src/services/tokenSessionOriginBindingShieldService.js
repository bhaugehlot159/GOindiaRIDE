...
const UserTokenSessionOriginBindingState = require('../models/UserTokenSessionOriginBindingState');
...
function resolveOptions() {
  const methods = splitCsv(env.tokenSessionOriginBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionOriginBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionOriginBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionOriginBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionOriginBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionOriginBindingRequireSidClaim),
    requireOriginSignal: Boolean(env.tokenSessionOriginBindingRequireOriginSignal),
    requireRefererSignal: Boolean(env.tokenSessionOriginBindingRequireRefererSignal),
    requireProtoSignal: Boolean(env.tokenSessionOriginBindingRequireProtoSignal),
    compareOriginHost: Boolean(env.tokenSessionOriginBindingCompareOriginHost),
    compareRefererHost: Boolean(env.tokenSessionOriginBindingCompareRefererHost),
    compareProto: Boolean(env.tokenSessionOriginBindingCompareProto),
    compareOriginRefererConsistency: Boolean(env.tokenSessionOriginBindingCompareOriginRefererConsistency),
    windowMs: normalizeNumber(env.tokenSessionOriginBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionOriginBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionOriginBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionOriginBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionOriginBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionOriginBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionOriginBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionOriginBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionOriginBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionOriginBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function parseOriginContext({ user = null, payload = {}, req = null }) {
  ...
  const originHost = normalizeHost(firstHeaderValue(req, ['origin']));
  const originHostHash = originHost ? hashValue(originHost) : '';
  const refererHost = normalizeHost(firstHeaderValue(req, ['referer', 'referrer']));
  const refererHostHash = refererHost ? hashValue(refererHost) : '';
...
function evaluateOriginBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
  ...
  if (options.requireOriginSignal && !context.originHost) {
    reasons.push('missing_origin_signal');
    missingClaimCount += 1;
  }
  if (options.requireRefererSignal && !context.refererHost) {
    reasons.push('missing_referer_signal');
    missingClaimCount += 1;
  }
  ...
  if (existing) {
    ...
    if (
      options.compareOriginHost
      && context.originHostHash
      && existing.boundOriginHostHash
      && String(existing.boundOriginHostHash) !== context.originHostHash
    ) {
      reasons.push('sid_origin_host_binding_drift');
    }

    if (
      options.compareRefererHost
      && context.refererHostHash
      && existing.boundRefererHostHash
      && String(existing.boundRefererHostHash) !== context.refererHostHash
    ) {
      reasons.push('sid_referer_host_binding_drift');
    }
  ...
  if (
    options.compareOriginRefererConsistency
    && context.originHostHash
    && context.refererHostHash
    && context.originHostHash !== context.refererHostHash
  ) {
    reasons.push('origin_referer_host_mismatch');
  }
  ...
  metadata.originHostHash = context.originHostHash;
  metadata.refererHostHash = context.refererHostHash;
...
function pickPrimaryReason(reasons = []) {
  ...
  if (reasons.includes('sid_origin_host_binding_drift')) return 'sid_origin_host_binding_drift';
  if (reasons.includes('sid_referer_host_binding_drift')) return 'sid_referer_host_binding_drift';
  if (reasons.includes('sid_proto_binding_drift')) return 'sid_proto_binding_drift';
  if (reasons.includes('origin_referer_host_mismatch')) return 'origin_referer_host_mismatch';
  if (reasons.includes('missing_sid_claim')) return 'missing_sid_claim';
  if (reasons.includes('missing_origin_signal')) return 'missing_origin_signal';
  if (reasons.includes('missing_referer_signal')) return 'missing_referer_signal';
  if (reasons.includes('missing_proto_signal')) return 'missing_proto_signal';
  if (reasons.length > 0) return reasons[0];
  return 'session_origin_binding_verified';
}

async function recordSessionOriginBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_origin_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_origin_binding_shield_triggered',
          note: payload.note || 'Token session origin binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionOriginBinding({ user = null, payload = null, req = null } = {}) {
  ...
  const context = parseOriginContext({ user, payload: payload || {}, req });
  if (!context.userId) {
    return { ok: true, reason: 'missing_user' };
  }

  if (options.requireSidClaim && !context.sid) {
    await recordSessionOriginBindingIncident(req, {
      eventType: 'session_origin_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session origin binding rejected token without sid',
  ...
  const existing = await UserTokenSessionOriginBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluateOriginBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionOriginBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          boundOriginHostHash: existing?.boundOriginHostHash || context.originHostHash,
          boundRefererHostHash: existing?.boundRefererHostHash || context.refererHostHash,
          ...
          lastOriginHostHash: context.originHostHash,
          lastRefererHostHash: context.refererHostHash,
          lastProto: context.proto,
          lastJtiHash: context.jtiHash,
          lastReason: evaluation.detected ? primaryReason : 'session_origin_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_origin_binding_verified' };
    }

    await recordSessionOriginBindingIncident(req, {
      eventType: 'session_origin_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session origin binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionOriginBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastProto: context.proto,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_origin_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_origin_binding_quarantined',
    req,
    metadata: {
      sidHash,
      path,
  ...
  await recordSessionOriginBindingIncident(req, {
    eventType: 'session_origin_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated session origin binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionOriginBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionOriginBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionOriginBindingState.find(query)
  ...
  return {
    ...
    items: rows.map((item) => ({
      ...
      boundOriginHostHash: String(item.boundOriginHostHash || ''),
      boundRefererHostHash: String(item.boundRefererHostHash || ''),
...
async function releaseTokenSessionOriginBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionOriginBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionOriginBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_origin_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionOriginBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionOriginBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_origin_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionOriginBinding,
  isTokenSessionOriginBindingSha256,
  getTokenSessionOriginBindingShieldSnapshot,
  releaseTokenSessionOriginBindingShieldStates,
  quarantineTokenSessionOriginBinding
};