...
const UserTokenSessionPlatformBindingState = require('../models/UserTokenSessionPlatformBindingState');
...
function normalizePlatformVersionTag(value) {
  const raw = normalizeLower(value, 40)
    .replace(/[^a-z0-9._-]/g, '');
  if (!raw) return '';
  return raw.slice(0, 24);
}

function parseAcceptPlatformVersion(value) {
  const text = String(value || '').trim();
  if (!text) return '';
  const first = text.split(',')[0] || '';
  const noQuality = first.split(';')[0] || '';
  return normalizePlatformTag(String(noQuality || '').replace(/"/g, ''));
}

function resolveOptions() {
  const methods = splitCsv(env.tokenSessionPlatformBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionPlatformBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionPlatformBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionPlatformBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionPlatformBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionPlatformBindingRequireSidClaim),
    requirePlatformSignal: Boolean(env.tokenSessionPlatformBindingRequirePlatformSignal),
    requirePlatformVersionSignal: Boolean(env.tokenSessionPlatformBindingRequirePlatformVersionSignal),
    comparePlatform: Boolean(env.tokenSessionPlatformBindingComparePlatform),
    comparePlatformVersion: Boolean(env.tokenSessionPlatformBindingComparePlatformVersion),
    allowPlatformVersionMajorFallback: Boolean(env.tokenSessionPlatformBindingAllowPlatformVersionMajorFallback),
    windowMs: normalizeNumber(env.tokenSessionPlatformBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionPlatformBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionPlatformBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionPlatformBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionPlatformBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionPlatformBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionPlatformBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionPlatformBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionPlatformBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionPlatformBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function parsePlatformContext({ user = null, payload = {}, req = null }) {
  ...
  const headerPlatform = normalizePlatformTag(
    firstHeaderValue(req, ['x-client-platform', 'x-platform', 'sec-ch-ua-platform'])
  );
  const headerPlatformVersion = normalizePlatformVersionTag(
    firstHeaderValue(req, ['x-client-platform-version', 'x-platform-version', 'sec-ch-ua-platform-version'])
  );
  const acceptLanguagePlatform = parseAcceptPlatformVersion(firstHeaderValue(req, ['accept-language']));
  const payloadPlatform = normalizePlatformTag(
    payload?.platform || payload?.devicePlatform || payload?.platformName || ''
  );
  const payloadPlatformVersion = normalizePlatformVersionTag(
    payload?.platformVersion || payload?.appVersion || payload?.devicePlatformVersion || ''
  );

  const platformTag = headerPlatform || payloadPlatform || acceptLanguagePlatform;
  const platformHash = platformTag ? hashValue(platformTag) : '';
  const platformVersionTag = headerPlatformVersion || payloadPlatformVersion;
...
function evaluatePlatformVersionMatch({ existingPlatformVersion = '', currentPlatformVersion = '', options }) {
  if (!options.comparePlatformVersion) return true;
  if (!existingPlatformVersion || !currentPlatformVersion) return true;
  if (existingPlatformVersion === currentPlatformVersion) return true;

  if (options.allowPlatformVersionMajorFallback) {
    const a = normalizePlatformVersionTag(existingPlatformVersion).split(/[._-]/)[0] || '';
    const b = normalizePlatformVersionTag(currentPlatformVersion).split(/[._-]/)[0] || '';
...
function evaluatePlatformBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
  ...
  if (options.requirePlatformSignal && !context.platformTag) {
    reasons.push('missing_platform_signal');
    missingClaimCount += 1;
  }
  if (options.requirePlatformVersionSignal && !context.platformVersionTag) {
    reasons.push('missing_platform_version_signal');
    missingClaimCount += 1;
  }
  ...
  if (existing) {
    ...
    if (
      options.comparePlatform
      && context.platformHash
      && existing.boundPlatformHash
      && String(existing.boundPlatformHash) !== context.platformHash
    ) {
      reasons.push('sid_platform_binding_drift');
    }

    const platformVersionMatch = evaluatePlatformVersionMatch({
      existingPlatformVersion: normalizeLower(existing.boundPlatformVersionTag || '', 40),
      currentPlatformVersion: normalizeLower(context.platformVersionTag || '', 40),
      options
    });
    if (!platformVersionMatch) {
      reasons.push('sid_platform_version_binding_drift');
    }
  }
  ...
  metadata.platformHash = context.platformHash;
  metadata.platformVersionTag = context.platformVersionTag;
...
function pickPrimaryReason(reasons = []) {
  ...
  if (reasons.includes('sid_platform_binding_drift')) return 'sid_platform_binding_drift';
  if (reasons.includes('sid_platform_version_binding_drift')) return 'sid_platform_version_binding_drift';
  if (reasons.includes('missing_sid_claim')) return 'missing_sid_claim';
  if (reasons.includes('missing_platform_signal')) return 'missing_platform_signal';
  if (reasons.includes('missing_platform_version_signal')) return 'missing_platform_version_signal';
  if (reasons.length > 0) return reasons[0];
  return 'session_platform_binding_verified';
}

async function recordSessionPlatformBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_platform_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_platform_binding_shield_triggered',
          note: payload.note || 'Token Session platform binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionPlatformBinding({ user = null, payload = null, req = null } = {}) {
  ...
  const context = parsePlatformContext({ user, payload: payload || {}, req });
  if (!context.userId) {
    return { ok: true, reason: 'missing_user' };
  }

  if (options.requireSidClaim && !context.sid) {
    await recordSessionPlatformBindingIncident(req, {
      eventType: 'session_platform_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session platform binding rejected token without sid',
  ...
  const existing = await UserTokenSessionPlatformBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluatePlatformBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionPlatformBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          boundPlatformHash: existing?.boundPlatformHash || context.platformHash,
          boundPlatformVersionTag: existing?.boundPlatformVersionTag || context.platformVersionTag,
          ...
          lastPlatformHash: context.platformHash,
          lastPlatformVersionTag: context.platformVersionTag,
          lastJtiHash: context.jtiHash,
          lastReason: evaluation.detected ? primaryReason : 'session_platform_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_platform_binding_verified' };
    }

    await recordSessionPlatformBindingIncident(req, {
      eventType: 'session_platform_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session platform binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionPlatformBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastPlatformVersionTag: context.platformVersionTag,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_platform_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_platform_binding_quarantined',
    req,
    metadata: {
      sidHash,
      path,
  ...
  await recordSessionPlatformBindingIncident(req, {
    eventType: 'session_platform_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated Session platform binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionPlatformBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionPlatformBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionPlatformBindingState.find(query)
  ...
  return {
    ...
    items: rows.map((item) => ({
      ...
      boundPlatformHash: String(item.boundPlatformHash || ''),
      boundPlatformVersionTag: String(item.boundPlatformVersionTag || ''),
...
async function releaseTokenSessionPlatformBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionPlatformBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionPlatformBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_platform_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionPlatformBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionPlatformBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_platform_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionPlatformBinding,
  isTokenSessionPlatformBindingSha256,
  getTokenSessionPlatformBindingShieldSnapshot,
  releaseTokenSessionPlatformBindingShieldStates,
  quarantineTokenSessionPlatformBinding
};
