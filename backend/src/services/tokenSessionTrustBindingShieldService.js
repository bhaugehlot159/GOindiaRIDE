...
const UserTokenSessionTrustBindingState = require('../models/UserTokenSessionTrustBindingState');
...
function resolveOptions() {
  const methods = splitCsv(env.tokenSessionTrustBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionTrustBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionTrustBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionTrustBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionTrustBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionTrustBindingRequireSidClaim),
    requireHostSignal: Boolean(env.tokenSessionTrustBindingRequireHostSignal),
    requireProtoSignal: Boolean(env.tokenSessionTrustBindingRequireProtoSignal),
    requireAsnSignal: Boolean(env.tokenSessionTrustBindingRequireAsnSignal),
    requireNetworkOrgSignal: Boolean(env.tokenSessionTrustBindingRequireNetworkOrgSignal),
    compareAsn: Boolean(env.tokenSessionTrustBindingCompareAsn),
    compareNetworkOrg: Boolean(env.tokenSessionTrustBindingCompareNetworkOrg),
    compareHost: Boolean(env.tokenSessionTrustBindingCompareHost),
    compareProto: Boolean(env.tokenSessionTrustBindingCompareProto),
    compareIpPrefix: Boolean(env.tokenSessionTrustBindingCompareIpPrefix),
    ipPrefixOctets: normalizeNumber(env.tokenSessionTrustBindingIpPrefixOctets, 3, 1, 8),
    windowMs: normalizeNumber(env.tokenSessionTrustBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionTrustBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionTrustBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionTrustBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionTrustBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionTrustBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionTrustBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionTrustBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionTrustBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionTrustBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function evaluateTrustBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
...
function pickPrimaryReason(reasons = []) {
  ...
  return 'session_trust_binding_verified';
}

async function recordSessionTrustBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_trust_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_trust_binding_shield_triggered',
          note: payload.note || 'Token session trust binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionTrustBinding({ user = null, payload = null, req = null } = {}) {
  ...
  if (options.requireSidClaim && !context.sid) {
    await recordSessionTrustBindingIncident(req, {
      eventType: 'session_trust_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session trust binding rejected token without sid',
  ...
  const existing = await UserTokenSessionTrustBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluateTrustBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionTrustBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          ...
          lastReason: evaluation.detected ? primaryReason : 'session_trust_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_trust_binding_verified' };
    }

    await recordSessionTrustBindingIncident(req, {
      eventType: 'session_trust_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session trust binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionTrustBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastIpPrefixHash: context.ipPrefixHash,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_trust_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_trust_binding_quarantined',
    req,
    metadata: {
      sidHash,
      reasons: evaluation.reasons
  ...
  await recordSessionTrustBindingIncident(req, {
    eventType: 'session_trust_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated session trust binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionTrustBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionTrustBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionTrustBindingState.find(query)
...
async function releaseTokenSessionTrustBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionTrustBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionTrustBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_trust_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionTrustBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionTrustBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_trust_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionTrustBinding,
