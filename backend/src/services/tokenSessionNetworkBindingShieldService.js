...
const UserTokenSessionNetworkBindingState = require('../models/UserTokenSessionNetworkBindingState');
...
function resolveOptions() {
  const methods = splitCsv(env.tokenSessionNetworkBindingMethods).map((item) => item.toLowerCase());
  return {
    enabled: Boolean(env.tokenSessionNetworkBindingShieldEnabled),
    failOpen: Boolean(env.tokenSessionNetworkBindingShieldFailOpen),
    protectedPrefixes: splitCsv(env.tokenSessionNetworkBindingProtectedPrefixes).map((item) => item.toLowerCase()),
    methods: methods.length ? methods : DEFAULT_METHODS,
    enforceOnAllMethods: Boolean(env.tokenSessionNetworkBindingEnforceOnAllMethods),
    requireSidClaim: Boolean(env.tokenSessionNetworkBindingRequireSidClaim),
    requireHostSignal: Boolean(env.tokenSessionNetworkBindingRequireHostSignal),
    requireProtoSignal: Boolean(env.tokenSessionNetworkBindingRequireProtoSignal),
    requireAsnSignal: Boolean(env.tokenSessionNetworkBindingRequireAsnSignal),
    requireNetworkOrgSignal: Boolean(env.tokenSessionNetworkBindingRequireNetworkOrgSignal),
    compareAsn: Boolean(env.tokenSessionNetworkBindingCompareAsn),
    compareNetworkOrg: Boolean(env.tokenSessionNetworkBindingCompareNetworkOrg),
    compareHost: Boolean(env.tokenSessionNetworkBindingCompareHost),
    compareProto: Boolean(env.tokenSessionNetworkBindingCompareProto),
    compareIpPrefix: Boolean(env.tokenSessionNetworkBindingCompareIpPrefix),
    ipPrefixOctets: normalizeNumber(env.tokenSessionNetworkBindingIpPrefixOctets, 3, 1, 8),
    windowMs: normalizeNumber(env.tokenSessionNetworkBindingViolationWindowMs, 20 * 60 * 1000, 60 * 1000, 24 * 60 * 60 * 1000),
    violationThreshold: normalizeNumber(env.tokenSessionNetworkBindingViolationThreshold, 3, 1, 50),
    missingClaimThreshold: normalizeNumber(env.tokenSessionNetworkBindingMissingClaimThreshold, 4, 1, 80),
    resetWindowOnPass: Boolean(env.tokenSessionNetworkBindingResetWindowOnPass),
    quarantineMs: normalizeNumber(env.tokenSessionNetworkBindingQuarantineMs, 2 * 60 * 60 * 1000, 60 * 1000, 7 * 24 * 60 * 60 * 1000),
    quarantineMaxMs: normalizeNumber(env.tokenSessionNetworkBindingQuarantineMaxMs, 24 * 60 * 60 * 1000, 10 * 60 * 1000, 30 * 24 * 60 * 60 * 1000),
    escalationFactor: normalizeNumber(env.tokenSessionNetworkBindingEscalationFactor, 2, 1, 5),
    autoRevokeSessions: Boolean(env.tokenSessionNetworkBindingAutoRevokeSessions),
    autoBanUser: Boolean(env.tokenSessionNetworkBindingAutoBanUser),
    recordTtlMs: normalizeNumber(env.tokenSessionNetworkBindingRecordTtlMs, 30 * 24 * 60 * 60 * 1000, 60 * 60 * 1000, 365 * 24 * 60 * 60 * 1000)
  };
}
...
function evaluateNetworkBindingDrift({ existing = null, context = {}, options, method, path }) {
  const reasons = [];
...
function pickPrimaryReason(reasons = []) {
  ...
  return 'session_network_binding_verified';
}

async function recordSessionNetworkBindingIncident(req, payload = {}) {
  try {
    ...
    await SecurityIncident.create({
      source: 'gateway',
      eventType: payload.eventType || 'session_network_binding_violation',
      ...
      timeline: [
        {
          action: 'token_session_network_binding_shield_triggered',
          note: payload.note || 'Token session network binding anomaly detected'
        }
      ]
    });
  } catch (_error) {
...
async function inspectUserTokenSessionNetworkBinding({ user = null, payload = null, req = null } = {}) {
  ...
  if (options.requireSidClaim && !context.sid) {
    await recordSessionNetworkBindingIncident(req, {
      eventType: 'session_network_binding_missing_sid',
      riskScore: 88,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session network binding rejected token without sid',
  ...
  const existing = await UserTokenSessionNetworkBindingState.findOne({ sidHash }).lean();
  if (existing) {
  ...
  const evaluation = evaluateNetworkBindingDrift({
    existing,
    context,
    options,
    method,
  ...
  if (!thresholdReached) {
    await UserTokenSessionNetworkBindingState.updateOne(
      { sidHash },
      {
        $set: {
          userId: context.userId,
          ...
          lastReason: evaluation.detected ? primaryReason : 'session_network_binding_verified',
    ...
    if (!evaluation.detected) {
      return { ok: true, reason: 'session_network_binding_verified' };
    }

    await recordSessionNetworkBindingIncident(req, {
      eventType: 'session_network_binding_violation',
      riskScore: 90,
      recommendedAction: 'deny_request',
      action: 'deny_request',
      note: 'Session network binding denied request',
      userId: context.userId,
      email: user?.email || null,
      signals: {
        path,
  ...
  await UserTokenSessionNetworkBindingState.updateOne(
    { sidHash },
    {
      $set: {
        ...
        lastIpPrefixHash: context.ipPrefixHash,
        lastJtiHash: context.jtiHash,
        lastReason: 'session_network_binding_quarantined',
        metadata: sanitizeMetadata({
          ...evaluation.metadata,
          reasons: evaluation.reasons,
          violationCount: nextViolationCount,
  ...
  await revokeUsersIfNeeded({
    ...
    boundUserId,
    reason: 'session_network_binding_quarantined',
    req,
    metadata: {
      sidHash,
      reasons: evaluation.reasons
  ...
  await recordSessionNetworkBindingIncident(req, {
    eventType: 'session_network_binding_quarantined',
    riskScore: 96,
    ...
    note: 'Repeated session network binding violations triggered quarantine',
    userId: context.userId,
    email: user?.email || null,
    signals: {
      path,
...
function isTokenSessionNetworkBindingSha256(value) {
  return /^[a-f0-9]{64}$/.test(String(value || '').trim().toLowerCase());
}

async function getTokenSessionNetworkBindingShieldSnapshot({ includeReleased = false, status = '', userId = '', sidHash = '', limit = 100 } = {}) {
  ...
  const rows = await UserTokenSessionNetworkBindingState.find(query)
...
async function releaseTokenSessionNetworkBindingShieldStates({ id = '', sidHash = '', userId = '', clearAll = false } = {}) {
  const normalizedId = String(id || '').trim();
  const normalizedSidHash = String(sidHash || '').trim().toLowerCase();
  const normalizedUserId = String(userId || '').trim();

  if (clearAll) {
    const result = await UserTokenSessionNetworkBindingState.updateMany(
      {},
      {
        $set: {
          status: 'released',
...
async function quarantineTokenSessionNetworkBinding({
  id = '',
  sidHash = '',
  userId = '',
  reason = 'admin_manual_session_network_binding_quarantine',
  durationMs = null,
  actorUserId = null,
  actorIp = '',
  metadata = {}
  ...
  const docs = await UserTokenSessionNetworkBindingState.find(query).select('userId sidHash').lean();
  ...
  const result = await UserTokenSessionNetworkBindingState.updateMany(
    query,
    {
      $set: {
        ...
        quarantineUntil,
        lastSeenAt: nowDate,
        lastReason: normalizeText(reason, 220) || 'admin_manual_session_network_binding_quarantine',
        metadata: sanitizeMetadata({
          ...(metadata || {}),
...
module.exports = {
  inspectUserTokenSessionNetworkBinding,
